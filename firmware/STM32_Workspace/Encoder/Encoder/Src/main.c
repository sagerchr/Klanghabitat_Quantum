...
int main(void)
{
  ...
  osThreadDef(I2C, ComunicationTask, osPriorityIdle, 0, 96);
...
void ENCODER_TASK(void const * argument)
{
  ...
  for(;;)
  {
	...
	if (count < Rintens){
		HAL_GPIO_WritePin(GPIOA, R_ENC1_Pin,GPIO_PIN_RESET);
	}
	else{
		HAL_GPIO_WritePin(GPIOA, R_ENC1_Pin,GPIO_PIN_SET);
	}
	if (count < Gintens){
		HAL_GPIO_WritePin(GPIOA, G_ENC1_Pin,GPIO_PIN_RESET);
	}
	else{
		HAL_GPIO_WritePin(GPIOA, G_ENC1_Pin,GPIO_PIN_SET);
	}
	if (count < Bintens){
		HAL_GPIO_WritePin(GPIOA, B_ENC1_Pin,GPIO_PIN_RESET);
	}
	else{
		HAL_GPIO_WritePin(GPIOA, B_ENC1_Pin,GPIO_PIN_SET);
	}
	count++;
  }
  /* USER CODE END 5 */ 
