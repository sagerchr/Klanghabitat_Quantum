...
void setup() {
   ...
   Serial2.begin(19200); //Komunikation zu DEBUGGER 
   ...
   LED1.begin();
   LED1.setPixelColor(0, LED1.Color(20,20,20)); LED1.setPixelColor(1, LED1.Color(0,0,0)); LED1.setPixelColor(2, LED1.Color(0,0,0)); LED1.setPixelColor(3, LED1.Color(0,0,0));
   LED1.show();
}
...
/******************************Wenn Header IST-WERT ist *********************************/
   if (StrINHeader == ist) //auslesen der IST-Wert für POTnumber
     ...
     if (firstbyteIST == 9) {IST_WERT = secondbyteIST +190;} //firstbyte = 9
        ...
        if (number == 0) {if (cc_wert_pot0 != IST_WERT && IST_WERT != lastSoll0) {MIDI.sendControlChange(MODE,IST_WERT,midichannel);sending = true; sending2 = true;} cc_wert_pot0 = IST_WERT;}
        if (number == 1) {if (cc_wert_pot1 != IST_WERT && IST_WERT != lastSoll1) {MIDI.sendControlChange(MODE,IST_WERT,midichannel);sending = true; sending2 = true;} cc_wert_pot1 = IST_WERT;}
        if (number == 2) {if (cc_wert_pot2 != IST_WERT) {MIDI.sendControlChange(number,IST_WERT,midichannel);sending = true; sending2 = true;} cc_wert_pot2 = IST_WERT;}            
...
int encoderweel1 (int Button1Pin)
{
  ...
  if (Button1 == HIGH && EncValue1 != EncValueLast1) {MIDI.sendControlChange (20,Position*42,midichannel); LED1.show();}
  if (Button1 == HIGH) {EncValueLast1 = EncValue1; Button1last = HIGH;}
  if (Button1 == LOW)  {EncValueLast1 = EncValuePushed1;LED1.show(); Button1last = LOW;}
