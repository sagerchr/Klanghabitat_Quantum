...
EncValue2 = EncValue/2;
  istPos = ((EncValue2 - left) * 129/right); //Der korrekte MIDI Wert!
  ...
  case 0:
    {
      ...
      if (EncValue == EncValueLast) {count = count+1;}

      if (EncValue != EncValueLast){count = 0;}
        
      if (count > 10) {matchleft = true;}
      
      if (matchleft == false)
      {  
        analogWrite(11,250); //links
        analogWrite(10,0); //rechts
        delay (1);
        analogWrite(11,210); //links
        analogWrite(10,0); //rechts
        delay (1);
  ...
  case 1:
    {
      ...
      if (count > 10) {matchright = true;}
      
      if (matchright == false)
      {  
        analogWrite(11,0); //links
        analogWrite(10,250); //rechts
        delay (1);
        analogWrite(11,0); //links
        analogWrite(10,210); //rechts
        delay (1);
      ...
      if (sollPos != istPos)
         {
            ...
            if (sollPos > istPos) //rechts Bewegung
              {
                 if (sollPos-istPos <= 100) //langsam fahren
                 {
                  analogWrite(11,0); //links
                  analogWrite(10,200); //rechts  
                  delayMicroseconds(1);  
                  analogWrite(11,0); //links
                  analogWrite(10,95); //rechts 
                  delayMicroseconds(1);  
                  analogWrite(11,0); //links
                  analogWrite(10,90); //rechts 
                 }
                 if (sollPos-istPos > 100) //schnell fahren
                 {
                  analogWrite(11,0); //links
                  analogWrite(10,130); //rechts  
                  delayMicroseconds(1);  
                  analogWrite(11,0); //links
                  analogWrite(10,125); //rechts 
                  delayMicroseconds(1);  
                  analogWrite(11,0); //links
                  analogWrite(10,120); //rechts
                 }
              }
            ...
            if (sollPos < istPos) //links Bewegung
              {
                if (istPos-sollPos <= 100) //langsam fahren
                { 
                  analogWrite(11,200); //links
                  analogWrite(10,0); //rechts  
                  delayMicroseconds(1);  
                  analogWrite(11,95); //links
                  analogWrite(10,0); //rechts 
                  delayMicroseconds(1);  
                  analogWrite(11,90); //links
                  analogWrite(10,0); //rechts 
                }
                if (istPos-sollPos > 100) //schnell fahren
                { 
                  analogWrite(11,130); //links
                  analogWrite(10,0); //rechts  
                  delayMicroseconds(1);  
                  analogWrite(11,125); //links
                  analogWrite(10,0); //rechts 
                  delayMicroseconds(1);  
                  analogWrite(11,120); //links
                  analogWrite(10,0); //rechts 
                }
