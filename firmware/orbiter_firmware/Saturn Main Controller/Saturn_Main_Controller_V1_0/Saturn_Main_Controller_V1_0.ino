...
void setup() {
   ...
   MIDI.begin(); 
   MIDI.turnThruOff();
   delay(10); //verzögern, damit NANO_POTs gebootet sind. 
}
...
/******************************Wenn Header IST-WERT ist *********************************/
   if (StrINHeader == ist) //auslesen der IST-Wert für POTnumber
     ...
     if (firstbyteIST == 9) {IST_WERT = secondbyteIST +190;} //firstbyte = 9
        ...
        if (number == 0) {if (cc_wert_pot0 != IST_WERT && IST_WERT != lastSoll0) {MIDI.sendControlChange(number,IST_WERT,midichannel);sending = true; sending2 = true;AnalogOutput2.setValue(1*((cc_wert_pot0)+900)); } cc_wert_pot0 = IST_WERT;}
        if (number == 1) {if (cc_wert_pot1 != IST_WERT && IST_WERT != lastSoll1) {MIDI.sendControlChange(number,IST_WERT,midichannel);sending = true; sending2 = true;AnalogOutput1.setValue(1*((cc_wert_pot1*20)+1900)); } cc_wert_pot1 = IST_WERT;}
