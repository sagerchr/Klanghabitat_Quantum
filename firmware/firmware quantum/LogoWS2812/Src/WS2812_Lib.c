...
void startDMA(void) {
	//uint8_t test[8] = {10};
 	dma_ready = 0;
	HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_3, (uint32_t *)WS2812_TIM_BUF, WS2812_BUFLEN);
	//HAL_Delay(1);
...
void WS2812_Refresh(void) {
	...
	while(!dma_ready);
	HAL_TIM_PWM_Stop(&htim3, TIM_CHANNEL_3); //THAT DOES THE TRICK!
